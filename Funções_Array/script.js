...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.help-icon').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            let text = '';
            switch (fn) {
                case 'insercao': text = 'Inserção: [] é simples; array_push aceita múltiplos; array_splice insere em posição; array_merge junta arrays; spread exige PHP 7.4+'; break;
                case 'remocao': text = 'Remoção: unset remove chave; array_pop/shift removem fim/início; splice remove por faixa; array_diff remove valores comparando outro array.'; break;
                case 'organizacao': text = 'Organização: sort/rsort ordenam; asort/ksort mantêm chaves; shuffle embaralha; array_reverse inverte.'; break;
                case 'funcoes': text = 'Funções comuns: array_map/array_filter/array_reduce para transformar/filtrar/reduzir; array_search/in_array para buscar; implode/explode para strings.'; break;
            }
            alert(text);
        });
    });
...
document.querySelectorAll('.help-icon').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        switch (fn) {
            case 'indexados':
                text = 'Arrays indexados usam índices numéricos automáticos. São úteis para listas simples como produtos, nomes ou IDs.';
                break;
            case 'insercao':
                text = 'Inserção: [] é simples; array_push aceita múltiplos; array_splice insere em posição; array_merge junta arrays; spread exige PHP 7.4+.';
                break;
            case 'remocao':
                text = 'Remoção: unset remove chave; array_pop/shift removem fim/início; splice remove por faixa; array_diff remove valores comparando outro array.';
                break;
            case 'organizacao':
                text = 'Organização: sort/rsort ordenam; asort/ksort mantêm chaves; shuffle embaralha; array_reverse inverte.';
                break;
            case 'funcoes':
                text = 'Funções comuns: array_map/array_filter/array_reduce para transformar/filtrar/reduzir; array_search/in_array para buscar; implode/explode para strings.';
                break;
        }
        mostrarPopupAjuda(text);
    });
});

// Novo: popup de ajuda expansível
function mostrarPopupAjuda(texto) {
    const modal = document.createElement('div');
    modal.className = 'popup-ajuda';
    modal.innerHTML = `
    ...
    modal.querySelector('.popup-fechar').addEventListener('click', () => modal.remove());
